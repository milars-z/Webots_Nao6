...
class SoccerSupervisor(Supervisor):
    def __init__(self):
        ...
        if not os.path.exists(DATA_FILE):
            with open(DATA_FILE, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["rel_x", "rel_y","angle","distance"]) # CSV header
    ...
    # This function is used for obstacle avoidance
    # Collect all player coordinates and compute pairwise differences
    # Generate and return a feature code
    # The feature code is a 6-digit ternary (base-3) number
    # Each digit represents one player
    # States: 0 = no obstacle, 1 = obstacle on left, 2 = obstacle on right
    def avoid_obs(self):
        ...
        avoid_code = 0
        threshold = 0.42 # distance threshold
        ...
        for i in range(len(players)):
            ...
            for j in range(len(players)):
                ...
                # state: left/right determination
                if dist < threshold:
    ...
    def ball_dis_cal(self):
        ...
        if self.kick_flag == 0:
            if speed > 0.5: 
                ...
                #print(f"Kick detected! Start measuring relative displacement: dx={self.dx:.3f}, dy={self.dy:.3f},angle={self.angle:.3f}")
            ...
            if speed < 0.02: 
                ...
                if self.total_dis > 0.1: 
                    #print(f"Ball stopped. Kick distance: {self.total_dis:.3f} meter")
