...
class GaitController:
    ...
    def __init__(self, Nao):
        ...
        # V1.2: Updated distributed walking motions
        self.motions = {
        ...
        # Use bitwise operation to record active motion flags
        self.flag = 0
        # Initialize motion timing arrays (current time and maximum duration)
        self.motion_time    = np.zeros(self.motion_sum)
        self.timemax = np.zeros(self.motion_sum)
        
        # Maximum duration for each motion
        # NOTE: When adding a new motion file, configure its duration here
        #       and ensure the order matches self.motions
        self.timemax = [
            2.6,  #forward
            2.6,  #back
            4.96, #turn_left
        ...
        # Create mapping from motion name to index
    ...
    # V1.1: Refactored state transition logic to reduce bugs
    def manage_state_(self, new_state):

        input_state = self.Init
        
        # Ignore invalid state input
        if new_state not in self.motions:
            return
        else:
            input_state = self.InputCheck

        # Ensure previous motion finishes before switching (V1.2 update)
        ...
        if input_state == self.FlagCheck or self.fist_run:
            ...
            # If previous frame was not forward and next frame is forward, trigger walk_start
